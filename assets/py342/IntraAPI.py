...
class intraAPI:
	...
	def getUserId(self, login):
		...
		params = {}
		r = self.get("/v2/users/" + str(login), headers=headers, params=params)
		jsObj = None
		jsObj = json.loads(r.content.decode("utf-8"))
		return jsObj['id']

	def getAchievement(self, achievementId):
		headers = {
			'Accept': 'application/json',
			'Authorization': 'Bearer ' + str(self.__token),
		}
		params = {}
		r = self.get("/v2/achievements/" + str(achievementId), headers=headers, params=params)
		jsObj = None
		jsObj = json.loads(r.content.decode("utf-8"))
		return jsObj
	...
	def getAchievementsUsers(self, userId):
		...
		int(r.headers['X-Per-Page']) * page <= int(r.headers['X-Total']):
			params['page'] = str(page)
			r = self.get("/v2/achievements_users?filter[user_id]=" + str(userId) , headers=headers, params=params)
			jsObj = json.loads(r.content.decode("utf-8"))
			for i in jsObj:
				achievementsUsers.append(i)
			page += 1
	...
	def getQuestsUsers(self, login):
		...
		int(r.headers['X-Per-Page']) * page <= int(r.headers['X-Total']):
			params['page'] = str(page)
			r = self.get("/v2/users/" + str(login) + "/quests_users", headers=headers, params=params)
			jsObj = json.loads(r.content.decode("utf-8"))
			for i in jsObj:
	...
	def createUser(self, body):
		...
		if not 'first_name' in body or not body['first_name']:
			...
			body['first_name'] = str(body['first_name'])
		if not 'last_name' in body or not body['last_name']:
			body['last_name'] = "Doe"
		else:
			body['last_name'] = str(body['last_name'])
